...
#if LLVM_VERSION_MAJOR <= 8 && (DETECT_ARCH_AARCH64 || defined (PIPE_ARCH_ARM) || DETECT_ARCH_S390 || DETECT_ARCH_MIPS64)
...
LLVMErrorRef module_transform(void *Ctx, LLVMModuleRef mod) {
   ...
   LLVMTargetMachineRef tm = LPJit::get_instance()->tm;
...
#if LLVM_VERSION_MAJOR >= 4 && (DETECT_ARCH_X86 || DETECT_ARCH_X86_64 || DETECT_ARCH_ARM)
...
gallivm_compile_module(struct gallivm_state *gallivm)
{
   ...
   gallivm_add_global_mapping(gallivm, gallivm->debug_printf_hook, (void *)debug_printf);

   lp_init_clock_hook(gallivm);
   gallivm_add_global_mapping(gallivm, gallivm->get_time_hook, (void *)os_time_get_nano);
